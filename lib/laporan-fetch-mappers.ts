...
export function mapCloudFinanceRow(
...
export function mapCloudKamarRow(row: Record<string, unknown>): ReportKamarRow {
...
export function mapCloudSurveyRow(row: Record<string, unknown>): SurveyCalonRow {
  return {
    id: String(row.id ?? ""),
    namaLengkap: String(row.nama_lengkap ?? ""),
    lokasiKos: String(row.lokasi_kos ?? ""),
...
export function buildPenghuniLookupMap(rows: Array<Record<string, unknown>>): Map<string, PenghuniLookup> {
  ...
  return penghuniLookupMap;
}

export function canSelectAllLokasiDanBlok(role: string): boolean {
  const r = String(role ?? "")
    .trim()
    .toLowerCase();
...
export function lokasiNamesForOwnerCloud(
...
export function unitNamesForOwnerCloud(
  units: string[],
  aksesBlokIds: string[],
