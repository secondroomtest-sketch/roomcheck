...
alter table public.penghuni add column if not exists deposit_kamar numeric not null default 0;
...
alter table public.penghuni add column if not exists booking_fee_paid boolean not null default false;
alter table public.penghuni add column if not exists booking_fee_nota text;
